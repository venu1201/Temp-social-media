...
const Popup = ({ data, name, changeremove, edit, type, setdefault }) => {
    ...
    return (
        <div key={key} className={`absolute "blur-lg":""}  overflow-hidden ss:ml-10  mt-2 top-[40px] z-[9999] ss:left-[20px] bg-gradient-to-r from-[#043d6e] via-[#1928b0] to-[#0f5085] h-[80%] ss:w-[85%] w-[100%]`}>
                    ...
                    <div className='mt-4 h-[85%] w-full overflow-scroll xs:px-4 px-2 '>
                        {
                            Fdata?.map((item, index) => (
                                <Peoplebox type={type} edit={edit} b={setb} key={index} setblur={setblur} data={item} setdefault={setdefault} />
                            ))
                        }
                    </div>
                ) : (
